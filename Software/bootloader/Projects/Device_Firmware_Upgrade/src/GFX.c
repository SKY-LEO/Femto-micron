...
void DispMode(uint8_t Mode){
	if(Mode == InvDisp) SB(InvDisp, Reg, 1);
	...
	case 0:
		for(XCnt = 0; XCnt<5; XCnt++){
			...
			for(YCnt = 0; YCnt<8; YCnt++){
				if(CChar & (1<<YCnt)) WritePix(X+XCnt, Y+YCnt, PixInv);
				else WritePix(X+XCnt, Y+YCnt, PixNorm);
			}
		}
		break;
	case 1:
		for(XCnt = 0; XCnt<10; XCnt++){
			for(YCnt = 0; YCnt<16; YCnt++){
				if(Chars[Ch+(XCnt>>1)] & (1<<(YCnt>>1))) WritePix(X+XCnt, Y+YCnt, Inv);
				else WritePix(X+XCnt, Y+YCnt, Inv);
			}
		}
		break;
	case 2:
		for(XCnt = 0; XCnt<15; XCnt++){
			for(YCnt = 0; YCnt<24; YCnt++){
				if(Chars[Ch+(XCnt/3)] & (1<<(YCnt/3))) WritePix(X+XCnt, Y+YCnt, Inv);
				else WritePix(X+XCnt, Y+YCnt, Inv);
			}
		}
		break;
