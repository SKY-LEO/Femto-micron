...
void MCP73831_state_detect(void)
{
  GPIO_InitTypeDef GPIO_InitStructure;
  uint32_t down, up, i;

  // Проверка PuPd_UP
  up = GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_9);
  ...
  GPIO_Init(GPIOA, &GPIO_InitStructure);
  for (i = 0; i < 10000; i++);
  down = GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_9);
  ...
  if((down == 0) && (up == 1))
	{
    Power.charging_state = HI_Z_State;
	} else if((down == 0) && (up == 0))
	{
    Power.charging_state = L_State;
	} else if((down == 1) && (up == 1))
	{
    Power.charging_state = H_State;
	} else
	{
		Power.charging_state = Unknown_State;
	}
}
