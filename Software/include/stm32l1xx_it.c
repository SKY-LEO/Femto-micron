      ...
      } else
      {
        if(Power.Display_active)
        {
          key |= 0x1;           // Кнопка Меnu
			...
			if(!Power.Display_active)
			{
				for (i = 0; i < 200000; i++)      // устранение дребезга кнопки
				{
					while (!GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_1) && button_holded == 0)
					{
						if(jitter > 120000)
						{
							button_holded = 1;
    ...
    if(!poweroff_state)
    {
      ...
      if((DataUpdate.Batt_update_time_counter > 2) && (Power.Display_active == ENABLE)) // Если дисплей активен, обновлять каждые 12 секунд
      {
        ...
        DataUpdate.Need_batt_voltage_update = ENABLE;
			...
			if(DataUpdate.mcp73831_update_time_counter > 0 && Power.USB_active == ENABLE)
      {
        DataUpdate.Need_mcp73831_state_update = ENABLE;
        DataUpdate.mcp73831_update_time_counter = 0;
      } else if (Power.USB_active == ENABLE)
			{
        DataUpdate.mcp73831_update_time_counter++;
			}
